...
public class travelUI_회원가입 {
	...
	public static void main(String[] args) {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (id.length()==0 || pw.length()==0 || name.length()==0) {
					JOptionPane.showMessageDialog(f, "필수입력사항을 모두 입력해주세요");
					t1.setText("");
					t2.setText("");
					t3.setText("");
		...
		btnNewButton.setBounds(359, 528, 592, 92);
		f.getContentPane().add(btnNewButton);
		
		JLabel lblNewLabel_2 = new JLabel("* : 필수입력사항");
		lblNewLabel_2.setFont(new Font("굴림", Font.PLAIN, 20));
		lblNewLabel_2.setBounds(170, 511, 152, 40);
